...
class MyDecisionTreeNode:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        """
        if (
            self._depth >= self._meta.max_depth or len(np.unique(y)) == 1 or len(y) < self._meta.min_samples_split
        ):
...
class MyDecisionTreeClassifier:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        predictions = [self.root.predict(sample) for sample in X]
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        probabilities = [self.root.predict_proba(sample) for sample in X]
